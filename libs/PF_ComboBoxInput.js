...
(function ($, mw, pf) {
        ...
        /**
         * Transform select/option to OOUI widget
         */
        ...
        pf.ComboBoxInput.prototype.setValues = function ( showAllValues = true ) {
            ...
            if (this.config.autocompletedatatype !== undefined) {
                ...
                var curString = this.getValue(); // current label or substring being typed
                ...
                if (data_type === 'cargo field') {
                    ...
                    my_server += "?action=pfautocomplete&format=json&cargo_table=" + table_and_field[0] + "&cargo_field=" + table_and_field[1] + "&substr=" + curString;
                    ...
                    if (this.config['autocompletesettings'] === 'external data') {
                        ...
                        if (wgPageFormsEDSettings[name].title !== undefined && wgPageFormsEDSettings[name].title !== "") {
                            ...
                            if (data.title !== undefined && data.title !== null) {
                                ...
                                data.title.forEach(function () {
                                    if (data.title[i] == curString ){
                                        self.itemFound = true;
                                    }
                                    if (wgPageFormsAutocompleteOnAllChars) {
                                        if (self.getConditionForAutocompleteOnAllChars(data.title[i], curString )) {
                                            values.push({
                                                data: data.title[i],
                                                label: data.title[i],
                                        ...
                                        if (self.checkIfAnyWordStartsWithInputValue(data.title[i], curString )) {
                                            values.push({
                                                data: data.title[i],
                                                label: data.title[i],
                                                highlighted: self.highlightText(data.title[i])
                        ...
                        if (Array.isArray(data) || typeof data == 'object') {
                            for (let key in data) {
                                ...
                                if ( 
                                    ( wgPageFormsAutocompleteOnAllChars && (this.getConditionForAutocompleteOnAllChars( optionLabel, curString )) )
                                    || ( this.checkIfAnyWordStartsWithInputValue(optionLabel, curString) )
                                    ) {
                                    values.push({
                                        data: optionVal,
                                        label: optionLabel,
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                response.pfautocomplete.forEach(function (item) {
                                    ...
                                    if ( item.displaytitle == curString || item.title == curString ) {
                                        self.itemFound = true;
                                    }
                                    if (wgPageFormsAutocompleteOnAllChars) {
                                        // dependent
                                        if (item.displaytitle !== undefined) {
                                            if (self.getConditionForAutocompleteOnAllChars( item.displaytitle, curString )){
                                                values.push({
                                            ...
                                            // no displaytitle
                                            if (self.getConditionForAutocompleteOnAllChars(item.title, curString )) {
                                                values.push({
                                                    data: item.title,
                                                    label: item.title,
                                        ...
                                        if (item.displaytitle !== undefined) {
                                            if (self.checkIfAnyWordStartsWithInputValue(item.displaytitle, curString)) {
                                                values.push({
                                                    data: item.displaytitle,
                                                    label: item.displaytitle,
                                                    highlighted: self.highlightText(item.displaytitle)
                                            ...
                                            if (self.checkIfAnyWordStartsWithInputValue(item.title, curString )) {
                                                values.push({
                                                    data: item.title,
                                                    label: item.title,
                                                    highlighted: self.highlightText(item.title)
        ...
        pf.ComboBoxInput.prototype.checkIfAnyWordStartsWithInputValue = function(string, curString ) {
            let wordSeparators = [
                '/', '(', ')', '|', 's'
            ].map( function(p) { return "\\" + p }).concat('^', '-', "'",'"');
            let regex = new RegExp('(' + wordSeparators.join('|') + ')' + curString.toLowerCase());
            return string.toLowerCase().match(regex) !== null;
        };
    
        pf.ComboBoxInput.prototype.getConditionForAutocompleteOnAllChars = function(string, curString ) {
            return string.toLowerCase().includes(curString.toLowerCase());
        };
         ...
         * @note - use getTitle not getLabel to get non-highlighted label
         */
        ...
        **/
         ...
         * @note not fully reality-proof as displaytitle may be 
         * identical to pagename
         * @param string newType
         */
        pf.ComboBoxInput.prototype.updateStringType = function ( newType ) {
            var className = 'string-type--' + newType;
            this.$input.removeClass( [ 'string-type--label', 'string-type--value' ] ).addClass( [ className ] );
